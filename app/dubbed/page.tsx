...
export default function DubbedAnimePage() {
  ...
  useEffect(() => {
    const fetchAnimes = async () => {
      try {
        setIsLoading(true)
        const response = await fetch("https://aninew-seven.vercel.app/dubbed-anime")
        if (!response.ok) {
  ...
  // Helper function to generate random studio
  function generateRandomStudio() {
    const studios = [
      "Mappa", "Ufotable", "Kyoto Animation", "Madhouse", 
      "Wit Studio", "Bones", "A-1 Pictures", "Production I.G", 
      "Sunrise", "Studio Ghibli", "Toei Animation", "Trigger"
    ]
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="mb-8">
        ...
        <p className="mt-2 text-gray-600 dark:text-gray-400">
          Enjoy your favorite anime with English voice acting
        </p>
      </div>
        ...
        <div className="flex flex-col md:flex-row gap-4">
          <div className="relative flex-grow">
            ...
            <input
              ...
              className="w-full rounded-md border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 pl-10 pr-10 py-2 text-black dark:text-white focus:outline-none focus:ring-2 focus:ring-orange-500"
            />
            {searchQuery && (
              <button 
          ...
          <div className="flex gap-2">
            ...
            <div className="flex rounded-md overflow-hidden border border-gray-200 dark:border-gray-700">
              <button 
                ...
                className={`px-3 py-2 text-sm ${filter === "all" ? "bg-orange-500 text-white" : "bg-white dark:bg-gray-900 text-gray-700 dark:text-gray-300"}`}
              >
                All
              </button>
              <button 
                onClick={() => setFilter("tv")} 
                className={`px-3 py-2 text-sm ${filter === "tv" ? "bg-orange-500 text-white" : "bg-white dark:bg-gray-900 text-gray-700 dark:text-gray-300"}`}
              >
                TV
              </button>
              <button 
                onClick={() => setFilter("movie")} 
                className={`px-3 py-2 text-sm ${filter === "movie" ? "bg-orange-500 text-white" : "bg-white dark:bg-gray-900 text-gray-700 dark:text-gray-300"}`}
              >
                Movie
              </button>
              <button 
                onClick={() => setFilter("ova")} 
                className={`px-3 py-2 text-sm ${filter === "ova" ? "bg-orange-500 text-white" : "bg-white dark:bg-gray-900 text-gray-700 dark:text-gray-300"}`}
              >
                OVA
              </button>
            </div>
            ...
            <div>
              ...
              <div className="flex rounded-md overflow-hidden border border-gray-200 dark:border-gray-700">
                <button 
                  ...
                  className={`px-3 py-1 text-sm ${sortBy === "latest" ? "bg-orange-500 text-white" : "bg-white dark:bg-gray-900 text-gray-700 dark:text-gray-300"}`}
                >
                  Latest
                </button>
                <button 
                  onClick={() => setSortBy("score")} 
                  className={`px-3 py-1 text-sm ${sortBy === "score" ? "bg-orange-500 text-white" : "bg-white dark:bg-gray-900 text-gray-700 dark:text-gray-300"}`}
                >
                  Rating
                </button>
                <button 
                  onClick={() => setSortBy("name")} 
                  className={`px-3 py-1 text-sm ${sortBy === "name" ? "bg-orange-500 text-white" : "bg-white dark:bg-gray-900 text-gray-700 dark:text-gray-300"}`}
                >
                  Name
                </button>
              </div>
      ...
      <div className="mb-4 flex items-center justify-between">
        <p className="text-sm text-gray-600 dark:text-gray-400">
          {isLoading 
            ? "Loading dubbed anime..." 
            : `Showing ${filteredAnimes.length} of ${animes.length} dubbed anime`
          }
        </p>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center py-12 px-4">
          <Volume2 className="h-16 w-16 text-gray-400 mb-4" />
          <h3 className="text-xl font-bold text-black dark:text-white">No dubbed anime found</h3>
            ...
            >
              <div className="relative">
                <div className="relative aspect-video overflow-hidden">
                  ...
                  {/* Language badge */}
                  <div className="absolute top-2 left-2 bg-orange-500 text-white text-xs px-2 py-1 rounded-md font-medium flex items-center gap-1 shadow-md">
                    <Volume2 className="h-3 w-3" />
                    <span>DUB</span>
                  </div>
                  
                  {/* Type badge */}
                  <div className="absolute top-2 right-2 bg-orange-500 text-white text-xs px-2 py-1 rounded-md font-medium shadow-md">
                    {anime.type}
                  </div>
                  
                  {/* Play overlay */}
                  <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-300">
                    <div className="p-2 rounded-full bg-orange-500/90 transform scale-75 group-hover:scale-100 transition-all duration-300">
              ...
              <div className="p-4">
                <h3 className="font-medium text-lg text-black dark:text-white line-clamp-1 group-hover:text-orange-500 transition-colors">
                  {anime.name}
                </h3>
                ...
                {anime.studio && (
                  <p className="text-xs text-gray-600 dark:text-gray-400 mb-2">
                    Studio: <span className="font-medium">{anime.studio}</span>
                  </p>
                )}
                ...
                <div className="flex gap-2">
                  <Link 
                    ...
                    className="flex-1 bg-orange-500 hover:bg-orange-600 text-white text-xs py-2 rounded text-center font-medium flex items-center justify-center gap-1 transition-colors"
                  >
                    <PlaySquare className="h-3 w-3" />
                    <span>Watch Dubbed</span>
                  </Link>
              ...
              {hoveredAnime === anime.id && (
                <div className="absolute left-full top-0 z-20 w-64 bg-white dark:bg-gray-800 p-3 rounded-lg shadow-xl border border-gray-200 dark:border-gray-700 ml-2 hidden lg:block">
                  ...
                  <p className="text-xs text-gray-600 dark:text-gray-400 mb-1">{anime.synopsis}</p>
                  {anime.studio && (
