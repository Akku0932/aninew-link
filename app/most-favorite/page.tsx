...
export default function MostFavoritePage() {
  ...
  const [sortBy, setSortBy] = useState<"favorites" | "score" | "name">("favorites")
  ...
  useEffect(() => {
    const fetchAnimes = async () => {
      try {
        setIsLoading(true)
        const response = await fetch("https://aninew-seven.vercel.app/most-favorite")
        if (!response.ok) {
        ...
        const enrichedData = Array.isArray(data) 
          ? data.map((anime, index) => ({
              ...
              favorites: anime.favorites || Math.floor(Math.random() * 500000) + 100000, // Random favorites between 100k and 600k
  ...
  // Helper function to format numbers (e.g., 100000 → 100K)
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="mb-8">
        ...
        <p className="mt-2 text-gray-600 dark:text-gray-400">
          The most beloved anime series by fans around the world
        </p>
      </div>
      ...
      <div className="mb-8 rounded-lg bg-gray-100 dark:bg-gray-800 p-4 shadow-md">
        <div className="flex flex-col md:flex-row gap-4">
          <div className="relative flex-grow">
            ...
            <input
              type="text"
              placeholder="Search most favorite anime..."
              value={searchQuery}
            ...
            <div>
              ...
              <div className="flex rounded-md overflow-hidden border border-gray-200 dark:border-gray-700">
                <button 
                  ...
                  className={`px-3 py-1 text-sm ${sortBy === "favorites" ? "bg-red-500 text-white" : "bg-white dark:bg-gray-900 text-gray-700 dark:text-gray-300"}`}
                >
                  Most Favorite
                </button>
                <button 
      ...
      <div className="mb-4 flex items-center justify-between">
        <p className="text-sm text-gray-600 dark:text-gray-400">
          {isLoading 
            ? "Loading most favorite anime..." 
            : `Showing ${filteredAnimes.length} of ${animes.length} most favorite anime`
          }
        </p>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center py-12 px-4">
          ...
          <h3 className="text-xl font-bold text-black dark:text-white">No favorite anime found</h3>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {filteredAnimes.map((anime, index) => (
            ...
            >
              ...
              {sortBy === "favorites" && index < 3 && (
                <div className="absolute top-0 left-0 z-10 w-12 h-12 overflow-hidden">
                  <div className={`absolute transform rotate-45 bg-gradient-to-br 
                    ${index === 0 ? "from-yellow-400 to-yellow-600" : 
                      index === 1 ? "from-gray-300 to-gray-500" : 
                      "from-amber-600 to-amber-800"} 
                    text-white w-16 text-center font-bold text-xs
                    -top-2 -left-6 py-1 shadow-md`}>
                    #{index + 1}
                  </div>
                </div>
              )}
              ...
              <div className="relative">
                <div className="relative aspect-video overflow-hidden">
                  ...
                  {/* Favorites badge */}
                  <div className="absolute top-2 right-2 bg-red-500 text-white text-xs px-2 py-1 rounded-md font-medium flex items-center gap-1 shadow-md">
                    <Heart className="h-3 w-3" fill="currentColor" />
                    <span>{formatNumber(anime.favorites || 0)}</span>
                  </div>
                  ...
                  <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-300">
                    <div className="p-2 rounded-full bg-red-500/90 transform scale-75 group-hover:scale-100 transition-all duration-300">
              ...
              <div className="p-4">
                <h3 className="font-medium text-lg text-black dark:text-white line-clamp-1 group-hover:text-red-500 transition-colors">
                  {anime.name}
                </h3>
                ...
                <div className="flex gap-2">
                  <Link 
                    href={`/info/${anime.id}`}
                    className="flex-1 bg-red-500 hover:bg-red-600 text-white text-xs py-2 rounded text-center font-medium flex items-center justify-center gap-1 transition-colors"
                  >
              ...
              {hoveredAnime === anime.id && (
                <div className="absolute left-full top-0 z-20 w-64 bg-white dark:bg-gray-800 p-3 rounded-lg shadow-xl border border-gray-200 dark:border-gray-700 ml-2 hidden lg:block">
                  <div className="flex justify-between items-center mb-1">
                    <h4 className="font-medium text-sm text-black dark:text-white">{anime.name}</h4>
