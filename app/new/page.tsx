...
export default function NewOnAninewPage() {
  ...
  useEffect(() => {
    const fetchAnimes = async () => {
      try {
        setIsLoading(true)
        const response = await fetch("https://aninew-seven.vercel.app/recently-added")
        if (!response.ok) {
          throw new Error("Failed to fetch recently added anime")
        }
        ...
        const enrichedData = Array.isArray(data) 
          ? data.map((anime, index) => ({
              ...
              addedDate: anime.addedDate || generateRandomAddedDate(),
  ...
  // Helper function to generate random added dates (within the last 2 weeks)
  function generateRandomAddedDate(): string {
    const now = new Date()
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="mb-8">
        ...
        <p className="mt-2 text-gray-600 dark:text-gray-400">
          Discover the latest anime series and movies added to our collection
        </p>
      </div>
        ...
        <div className="flex flex-col md:flex-row gap-4">
          <div className="relative flex-grow">
            ...
            <input
              ...
              className="w-full rounded-md border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 pl-10 pr-10 py-2 text-black dark:text-white focus:outline-none focus:ring-2 focus:ring-teal-500"
            />
            {searchQuery && (
              <button 
          ...
          <div className="flex gap-2">
            ...
            <div className="flex rounded-md overflow-hidden border border-gray-200 dark:border-gray-700">
              <button 
                ...
                className={`px-3 py-2 text-sm ${filter === "all" ? "bg-teal-500 text-white" : "bg-white dark:bg-gray-900 text-gray-700 dark:text-gray-300"}`}
              >
                All
              </button>
              <button 
                onClick={() => setFilter("tv")} 
                className={`px-3 py-2 text-sm ${filter === "tv" ? "bg-teal-500 text-white" : "bg-white dark:bg-gray-900 text-gray-700 dark:text-gray-300"}`}
              >
                TV
              </button>
              <button 
                onClick={() => setFilter("movie")} 
                className={`px-3 py-2 text-sm ${filter === "movie" ? "bg-teal-500 text-white" : "bg-white dark:bg-gray-900 text-gray-700 dark:text-gray-300"}`}
              >
                Movie
              </button>
              <button 
                onClick={() => setFilter("ova")} 
                className={`px-3 py-2 text-sm ${filter === "ova" ? "bg-teal-500 text-white" : "bg-white dark:bg-gray-900 text-gray-700 dark:text-gray-300"}`}
              >
                OVA
              </button>
            </div>
            ...
            <div>
              ...
              <div className="flex rounded-md overflow-hidden border border-gray-200 dark:border-gray-700">
                <button 
                  ...
                  className={`px-3 py-1 text-sm ${sortBy === "latest" ? "bg-teal-500 text-white" : "bg-white dark:bg-gray-900 text-gray-700 dark:text-gray-300"}`}
                >
                  Latest
                </button>
                <button 
                  onClick={() => setSortBy("score")} 
                  className={`px-3 py-1 text-sm ${sortBy === "score" ? "bg-teal-500 text-white" : "bg-white dark:bg-gray-900 text-gray-700 dark:text-gray-300"}`}
                >
                  Rating
                </button>
                <button 
                  onClick={() => setSortBy("name")} 
                  className={`px-3 py-1 text-sm ${sortBy === "name" ? "bg-teal-500 text-white" : "bg-white dark:bg-gray-900 text-gray-700 dark:text-gray-300"}`}
                >
                  Name
                </button>
              </div>
      ...
      <div className="mb-4 flex items-center justify-between">
        <p className="text-sm text-gray-600 dark:text-gray-400">
          {isLoading 
            ? "Loading newest anime..." 
            : `Showing ${filteredAnimes.length} of ${animes.length} newly added anime`
          }
        </p>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center py-12 px-4">
          ...
          <h3 className="text-xl font-bold text-black dark:text-white">No new anime found</h3>
            ...
            >
              <div className="relative">
                <div className="relative aspect-video overflow-hidden">
                  ...
                  {/* New badge */}
                  <div className="absolute top-2 right-2 bg-teal-500 text-white text-xs px-2 py-1 rounded-md font-medium flex items-center gap-1 shadow-md">
                    <PlusCircle className="h-3 w-3" />
                    <span>NEW</span>
                  </div>
                  
                  {/* Type badge */}
                  <div className="absolute top-2 left-2 bg-teal-500 text-white text-xs px-2 py-1 rounded-md font-medium shadow-md">
                    {anime.type}
                  </div>
                  
                  {/* Play overlay */}
                  <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-300">
                    <div className="p-2 rounded-full bg-teal-500/90 transform scale-75 group-hover:scale-100 transition-all duration-300">
                ...
                <div className="absolute bottom-0 left-0 right-0 p-3 flex justify-between items-center">
                  ...
                  {anime.addedDate && (
                    <div className="flex items-center gap-1 bg-black/70 text-white px-2 py-1 rounded text-xs backdrop-blur-sm">
                      ...
                      <span>{getRelativeTimeString(anime.addedDate)}</span>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="p-4">
                <h3 className="font-medium text-lg text-black dark:text-white line-clamp-1 group-hover:text-teal-500 transition-colors">
                  {anime.name}
                </h3>
                ...
                <div className="flex gap-2">
                  <Link 
                    href={`/info/${anime.id}`}
                    className="flex-1 bg-teal-500 hover:bg-teal-600 text-white text-xs py-2 rounded text-center font-medium flex items-center justify-center gap-1 transition-colors"
                  >
                ...
                <div className="absolute left-full top-0 z-20 w-64 bg-white dark:bg-gray-800 p-3 rounded-lg shadow-xl border border-gray-200 dark:border-gray-700 ml-2 hidden lg:block">
                  ...
                  {anime.addedDate && (
                    <p className="text-xs text-gray-600 dark:text-gray-400 mb-1 flex items-center gap-1">
                      ...
                      <span>Added {anime.addedDate}</span>
                    </p>
                  )}
